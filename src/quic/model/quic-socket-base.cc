...
namespace ns3
{
  ...
  int QuicSocketBase::Bind(const Address &address)
  {
    NS_LOG_FUNCTION(this);
  ...
  int QuicSocketBase::Listen(void)
  {
    NS_LOG_FUNCTION(this);
  ...
  int QuicSocketBase::Connect(const Address &address)
  {
    NS_LOG_FUNCTION(this);
    ...
    if (result != m_quicl4->GetAuthAddresses().end() || m_quicl4->Is0RTTHandshakeAllowed())
    {
      NS_LOG_INFO(
  ...
  int QuicSocketBase::Send(Ptr<Packet> p, uint32_t flags)
  {
  ...
  int QuicSocketBase::Send(Ptr<Packet> p)
  {
    NS_LOG_FUNCTION(this);
  ...
  QuicSocketBase::SendDataPacket(SequenceNumber32 packetNumber, uint32_t maxSize, bool withAck, uint8_t pathId)
  {
    ...
    if (m_subflows[pathId]->m_tcb->m_pacing)
    {
      ...
      if (m_pacingTimer.IsExpired())
      {
        ...
        NS_LOG_DEBUG("Pacing Timer is in expired state, activate it. Expires in " << m_subflows[pathId]->m_tcb->m_pacingRate.Get().CalculateBytesTxTime(sz));
    ...
    else if (m_socketState == OPEN)
    {
      ...
      else if (!m_connected and m_quicl4->Is0RTTHandshakeAllowed())
      {
        ...
        m_connected = true;
        m_keyPhase == QuicHeader::PHASE_ONE ? m_keyPhase = QuicHeader::PHASE_ZERO : m_keyPhase = QuicHeader::PHASE_ONE;
      }
      else
      {
        head = QuicHeader::CreateShort(m_connectionId, packetNumber, !m_omit_connection_id, m_keyPhase);
    ...
    if (!m_quicCongestionControlLegacy)
    {
      DynamicCast<QuicCongestionOps>(m_congestionControl)->OnPacketSent(m_subflows[pathId]->m_tcb, packetNumber, isAckOnly);
    }
    if (!isAckOnly)
    {
  ...
  QuicSocketBase::SetReTxTimeout(uint8_t pathId)
  {
    ...
    {
      NS_LOG_LOGIC("RTO");
      alarmDuration = m_subflows[pathId]->m_tcb->m_smoothedRtt + 4 * m_subflows[pathId]->m_tcb->m_rttVar + m_subflows[pathId]->m_tcb->m_maxAckDelay;
  ...
  int QuicSocketBase::SendTo(Ptr<Packet> p, uint32_t flags, const Address &address)
  {
    NS_LOG_FUNCTION(this);
  ...
  int QuicSocketBase::Close(void)
  {
    ...
    if (m_idleTimeoutEvent.IsRunning() and m_socketState != IDLE and m_socketState != CLOSING) // Connection Close from application signal
    {
    ...
    else if (m_idleTimeoutEvent.IsExpired() and m_socketState != CLOSING and m_socketState != IDLE and m_socketState != LISTENING) // Connection Close due to Idle Period termination
    {
      SetState(CLOSING);
      m_drainingPeriodEvent.Cancel();
      NS_LOG_LOGIC(
    ...
    else if (m_idleTimeoutEvent.IsExpired() and m_drainingPeriodEvent.IsExpired() and m_socketState != CLOSING and m_socketState != IDLE) // close last listening sockets
    {
      NS_LOG_LOGIC(this << " Closing listening socket");
      DoClose();
    }
    else if (m_idleTimeoutEvent.IsExpired() and m_drainingPeriodEvent.IsExpired() and m_socketState == IDLE)
    {
  ...
  int QuicSocketBase::ShutdownSend(void)
  {
    NS_LOG_FUNCTION(this);
  ...
  int QuicSocketBase::ShutdownRecv(void)
  {
    NS_LOG_FUNCTION(this);
  ...
  int QuicSocketBase::GetSockName(Address &address) const
  {
    NS_LOG_FUNCTION(this);
  ...
  int QuicSocketBase::GetPeerName(Address &address) const
  {
    NS_LOG_FUNCTION(this);
  ...
  int QuicSocketBase::SetupCallback(void)
  {
    NS_LOG_FUNCTION(this);
  ...
  int QuicSocketBase::AppendingRx(Ptr<Packet> frame, Address &address)
  {
  ...
  QuicSocketBase::OnSendingAckFrame(uint8_t pathId)
  {
         ...
         ++curr_rec_it, ++next_rec_it)
    {
      ...
      if (((*curr_rec_it) - (*next_rec_it) - 1 > 0) and ((*curr_rec_it) != (*next_rec_it)))
      {
  ...
  QuicSocketBase::OnReceivedAckFrame(QuicSubheader &sub)
  {
    ...
    NS_ABORT_MSG_IF(
        ackBlockCount != additionalAckBlocks.size() and ackBlockCount != gaps.size(),
    ...
    if (m_subflows[pathId]->m_tcb->m_rtoCount > 0)
    {
      ...
      if (m_subflows[pathId]->m_tcb->m_largestSentBeforeRto.GetValue() < largestAcknowledged)
      {
        ...
        uint32_t newPackets = (largestAcknowledged - m_subflows[pathId]->m_tcb->m_largestSentBeforeRto.GetValue()) / GetSegSize();
    ...
    if (!lostPackets.empty())
    {
      ...
      {
        DynamicCast<QuicCongestionOps>(m_congestionControl)->OnPacketsLost(m_subflows[pathId]->m_tcb, lostPackets);
      }
      DoRetransmit(lostPackets, pathId);
    }
    /* else */
    if (ackedBytes > 0)
    {
      ...
      if (!m_quicCongestionControlLegacy)
      {
        ...
        // Process the ACK
        if (m_enableMultipath && m_ccType != QuicNewReno)
        {
          m_subflows[pathId]->m_tcb->m_bytesBeforeLost2 += ackedBytes;
          double alpha = GetOliaAlpha(pathId, m_ccType);
          double sum_rate = 0;
      ...
      else
      {
        ...
        if (m_subflows[pathId]->m_tcb->m_congState != TcpSocketState::CA_RECOVERY && m_subflows[pathId]->m_tcb->m_congState != TcpSocketState::CA_LOSS)
        {
      ...
      QuicTransportParameters transportParameters)
  {
    ...
    uint32_t mask = transportParameters.GetInitialMaxStreamIdBidi() & 0x00000003;
    ...
    if (transportParameters.GetMaxPacketSize() < QuicSocketBase::MIN_INITIAL_PACKET_SIZE or transportParameters.GetMaxPacketSize() > 65527)
    {
      AbortConnection(
          QuicSubheader::TransportErrorCodes_t::TRANSPORT_PARAMETER_ERROR,
  ...
  int QuicSocketBase::DoConnect(void)
  {
    NS_LOG_FUNCTION(this);
  ...
  int QuicSocketBase::DoFastConnect(void)
  {
    NS_LOG_FUNCTION(this);
  ...
  int QuicSocketBase::DoClose(void)
  {
    NS_LOG_FUNCTION(this);
  ...
  {
    if (version == QUIC_VERSION || version == QUIC_VERSION_DRAFT_10 || version == QUIC_VERSION_NS3_IMPL)
    {
      return true;
    }
    else
                                  ...
                                  bool applicationClose)
  {
    ...
    case OPEN:
      quicHeader =
          !m_connected ? QuicHeader::CreateHandshake(m_connectionId, m_vers,
                                                     m_subflows[0]->m_tcb->m_nextTxSequence++)
  ...
  double QuicSocketBase::GetOliaAlpha(uint8_t pathId, CcType_t Olia_factor)
  {
    ...
    for (uint8_t i = 0; i < m_subflows.size(); i++)
    {
      ...
      double rate = std::max(m_subflows[i]->m_tcb->m_bytesBeforeLost1,
                             m_subflows[i]->m_tcb->m_bytesBeforeLost2) /
                    pow(m_subflows[i]->m_tcb->m_lastRtt.Get().GetSeconds(), 2);
    ...
    double pathRtt = m_subflows[pathId]->m_tcb->m_lastRtt.Get().GetSeconds();

    if (m_subflows[pathId]->m_tcb->m_minRtt == Seconds(0))
      m_subflows[pathId]->m_tcb->m_minRtt = Seconds(9999);

    m_subflows[pathId]->m_tcb->m_minRtt = std::min(
        m_subflows[pathId]->m_tcb->m_minRtt,
        m_subflows[pathId]->m_tcb->m_lastRtt.Get());

    double rttMin = m_subflows[pathId]->m_tcb->m_minRtt.GetSeconds();
    double rttRatio = pathRtt / rttMin;
    ...
    {
    case OLIA6:
      factor = 0.6 + (2.0 - 0.6) * std::exp(-now / 2.0);
      break;
    ...
    case OLIA8:
    {
      // === 改良 alpha: 平滑 RTT ratio 調整 ===
      factor = 1.8 + 2.0 * std::exp(-rttRatio / 2.5); // Range: ~3.8 → 1.8

      // === 對所有 path 按 RTT 權重分配 alpha ===
      double totalWeight = 0.0;
      std::vector<double> weights;
      for (const auto &sf : m_subflows)
      {
        double rtt = sf->m_tcb->m_lastRtt.Get().GetSeconds();
        double weight = 1.0 / (rtt * rtt + 1e-6); // 小 RTT 得高權重
      ...
      if (totalWeight > 0)
      {
        return factor * (weights[pathId] / totalWeight);
      }
      else
      {
        return 0.0;
