...
dataTotal = pd.DataFrame (comTime, columns = ['one', 'two', 'four',  '0116-3',  '0126', '0128'])
...
ticks = ['one', 'two', 'four',   '0116-3', '0126', '0128']
...
for box in ct_plot5['boxes']:
    box.set(hatch = '\\', fill=False)
ct_plot6 = plt.boxplot(ct6,positions=np.array(np.arange(len(ct6)))+bar_width*4+0.1*4,widths=bar_width, patch_artist=True, boxprops=boxprops, whiskerprops=whiskerprops, capprops=capprops, medianprops = medianprops)
for box in ct_plot6['boxes']:
    box.set(hatch = 'X', fill=False)
ct_plot7 = plt.boxplot(ct7,positions=np.array(np.arange(len(ct6)))+bar_width*5+0.1*5,widths=bar_width, patch_artist=True, boxprops=boxprops, whiskerprops=whiskerprops, capprops=capprops, medianprops = medianprops)
...
route = '../results-wns3/scalable_comtime_0128.png'
