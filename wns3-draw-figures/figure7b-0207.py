...
def help0207(num,i):
    dir = topDir+'two-0207-'+str(num)
    file = open(dir+'/scheduler'+str(i)+'-queue.txt', 'r')
    finish_time = float(file.readlines()[-2].split('\t')[0])
    file = open(dir+'/scheduler'+str(i)+'-rx-0207.txt', 'r')
    lines = file.readlines()
    goodput = []
    c_time = []
    for line in lines:
...
plt.plot(two_0207['Time'], two_0207['goodput'], color="black", label="two_0207 paths", linewidth=6)
...
route = '../results-wns3/ins_throughput_scalable-0207.png'
