...
def help0128(num,i):
    dir = topDir+'two-0128-'+str(num)
    file = open(dir+'/scheduler'+str(i)+'-queue.txt', 'r')
    finish_time = float(file.readlines()[-2].split('\t')[0])
    file = open(dir+'/scheduler'+str(i)+'-rx-0128.txt', 'r')
    lines = file.readlines()
    goodput = []
    c_time = []
    for line in lines:
...
def help0209(num,i):
    dir = topDir+'two-0209-'+str(num)
    file = open(dir+'/scheduler'+str(i)+'-queue.txt', 'r')
    finish_time = float(file.readlines()[-2].split('\t')[0])
    file = open(dir+'/scheduler'+str(i)+'-rx-0209.txt', 'r')
    lines = file.readlines()
    goodput = []
    c_time = []
    for line in lines:
        temp = line.split('\t')
        if (temp[0] == '7' and c_time == []):
            c_time.append(float(temp[1]))
            c_time.append(float(temp[5]))
        if (temp[0] == '11' and len(c_time) == 2):
            c_time[1] = c_time[1] + float(temp[5])
            c_time.append(i)
            goodput.append(c_time)
            if (c_time[0] > finish_time):
...
two_0128 = help0128(j,0)
two_0209 = help0209(j,0)
four = helpfour(j,0)  
...
plt.plot(two_0209['Time'], two_0209['goodput'], color="black", label="two_0209 paths", linewidth=6)
plt.plot(two_0128['Time'], two_0128['goodput'], color="#d233c2", label="two_0128 paths", linewidth=4)
...
route = '../results-wns3/ins_throughput_scalable-0209.png'
