...
def helpltenrtcp(num,i):
    dir = topDir+'two-ltenr-tcp-'+str(num)
    file = open(dir+'/scheduler'+str(i)+'-queue.txt', 'r')
    finish_time = float(file.readlines()[-1].split('\t')[0])
    file = open(dir+'/scheduler'+str(i)+'-rx-ltenr-tcp.txt', 'r')
    lines = file.readlines()
    goodput = []
    c_time = []
    for line in lines:
...
plt.plot(ltenr['Time'], ltenr['goodput'], color="orange", label="ltenr paths", linewidth=4)
...
path = '../results-wns3/ins_throughput_scalable-ltenr-tcp.png'
